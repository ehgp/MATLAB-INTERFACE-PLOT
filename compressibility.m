...
% critical properties from http://encyclopedia.airliquide.com/Encyclopedia.asp
    ...
    case {'chlorodifluoromethane' 'CHClF2' 'chclf2'}
        Tc = 369.3; % [K] critical temperature
        Pc = 49.9; % [bar] critical pressure
...
Z = .01:.0001:1; % range of reasonable Z values 
...
if length(Tr)~=length(Pr) && min([length(Tr) length(Pr)])~= 1
    errordlg('Temperature and pressure array lengths must match if neither is a scalar.')
end
